...
if (isset($_POST["create"])) {
	try {
		...
		$stmt->execute(array(':u'=>$username,':p'=>$password,':l'=>$level,
		':in'=>$is_new,':ia'=>$is_admin,
		));
		$pdo->commit();
	} catch (Exception $e) {
		$pdo->rollBack();
		$_SESSION["create_error"] = $e;//"No se pudo agregar el nuevo ingreso";
		header("Location: $url/crud/usuarios/read_user.php");
		exit;
	}
	$_SESSION["create_successful"] = "El detalle de usuarios se actualizo adecuadamente";
	header("Location: $url/crud/usuarios/read_user.php");
	exit;
}
?>
	...
	<div class="container">
		<div class="mt-auto d-flex justify-content-center align-items-center">
			...
			<form method="post" style="max-width: 600px;">
					...
					<button type="submit" class="w-100 btn btn-danger mb-3" name="create" value="create">Confirmar</button>
